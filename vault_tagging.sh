...
if if_active==true; then
    ...
    sudo aws ec2 create-tags --region $region --resources $instance_id --tags Key='Patch Group',Value='prd-rhel-security-critical-second'
else
    echo 'this is passive node, tag first'
    sudo aws ec2 create-tags --region $region --resources $instance_id --tags Key='Patch Group',Value='prd-rhel-security-critical-first'
fi
